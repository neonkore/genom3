...
in    \xref{sec:iff:grammar}{section~\ref{sec:iff:grammar}}    and   associated
...
than white space.  The preprocessing is done by the C preprocessor available on
the host system and configured during the build of \GenoM{}. It is invoked as a
separate process.
...
\label{sec:iff:grammar}

\input{dotgen-rules}

\section{Dotgen specification}
\label{sec:iff:dgspec}
...
These directives are normally not used, and are the result of the preprocessing
of the file (section~\ref{sec:iff:preproc}). They can be used mostly to achieve
special effects on error reporting or similar.
...
file, optionally  followed by a flag. The  flag is never used  by \GenoM{}. Its
meaning depends  on the C  preprocessor used: see  for instance chapter  1.8, C
preprocessor output, of the FSF C preprocessor~\cite{cpp|fsf}.

The file  name and  line number  replace the current  value kept  internally by
\GenoM{}  and and  used in  error reporting  messages as  well as  a  few other
places.


\section{Identifiers and reserved keywords}
\label{sec:iff:identifier}

An identifier is  a sequence of ASCII alphabetic,  digit, and underscore ("\_")
characters.  The first  character must  be an  ASCII alphabetic  character.

\smallbreak\input{dotgen-rule-identifier}

Keywords that are reserved words in the dotgen language are valid identifiers
...
A module construct is used to scope IDL identifiers. It is similar to a C++
...
\smallbreak\input{dotgen-rule-type_dcl}
